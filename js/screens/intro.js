...
// Intro = prawdziwa animacja Izabeli (render z Veo). Po obejrzeniu → Paszport Fonetyczny.
export function renderIntro(mount) {
  const goNext = () => navigate('#/phonetic');
  ...
  // Nakładka „Odtwórz" — gwarantuje dźwięk (autoplay z dźwiękiem bywa blokowany)
  ...
  mount.append(
    el('div.intro-video-wrap.fade-in', { style: 'position:relative' }, [
      ...
      el('p.muted', { style: 'text-align:center;margin:0', text: 'Izabela właśnie wylądowała… 🚀' }),
      startBtn,
    ])
  );

  // Spróbuj odtworzyć od razu; jeśli przeglądarka zablokuje — zostaje nakładka „Odtwórz".
